...
Next, we used selection models to approximate the selection process of experiments into the meta-analysis under the assumption that experiments that observe significant effects (i.e., with low p-values) are more likely to get published. We ran one selection model assuming a weight function with fixed p-value cutoffs at p = .01, p = .05, and p = 0.3[@vevea1995], as well as multiple models with a priori selection functions that assume bias of varying severity [@vevea2005]. These analyses show that our results are robust to small to medium amounts of publication bias (Supplementary Tables 8 and 9).
...
```{r, trim_fill_selection}
# Re-fit frequentist meta-analysis using a two-level model
res_freq_2 <- rma.uni(yi = dat$gi, vi = dat$vi)

# Run trim-and-fill analysis
res_trim_fill <- trimfill(res_freq_2)
print(res_trim_fill)

# Easy weight function selection model (see metafor docs and Lauer et al., 2019)
res_selection_weights <- selmodel(
  res_freq_2,
  type = "stepfun", steps = c(0.01, .05, .3, 1)
)
print(res_selection_weights)
...
print(res_selection_steps)
```

```{r, tabs8, include=TRUE}
# Combine all trim-and-fill and selection models into one table
tibble(
  res = c(
    list(res_freq_2, res_trim_fill, res_selection_weights), res_selection_steps
  ),
  label_1 = c(
  ...
  pmap_dfr(
    ~ print_res_freq_table(
      ...
      label_col_1 = "Model", label_col_2 = " ", print_sigma2s = FALSE
    )
  ) %>%
  select(-Parameter) %>%
  rename(`Hedges' $g$` = Estimate) -> tab_trim_fill_selection

# Save the table
write_csv(
  tab_trim_fill_selection,
  file = here(tables_dir, "trim_fill_selection.csv")
)
...
apa_table(
  tab_trim_fill_selection,
  align = "llccccc",
  caption = "Trim-and-fill and selection model analyses",
  font_size = "scriptsize",
  escape = FALSE
)
```

```{r, figs4, include=TRUE, fig.width=6, fig.height=3.5, out.width="50%"}
# Compute new funnel, taking into account the trim-and-fill results
funnel_trim_fill <- with(
  list(
    z_crit_05 = stats::qnorm(0.975),
    max_se = max(sqrt(res_trim_fill$vi)) + 0.05
  ),
  {
    data.frame(
      x = c(
        res_trim_fill$b - z_crit_05 * sqrt(max_se^2),
        res_trim_fill$b,
        res_trim_fill$b + z_crit_05 * sqrt(max_se^2)
      ),
      y = c(max_se, 0, max_se)
...
# Create new funnel plot, enhanced by the filled in studies
trim_fill_color <- "#e42536"
plots_funnel[[1]] +
  ...
  geom_vline(xintercept = res_trim_fill$b, color = trim_fill_color) +
  # Add filled in studies
  geom_point(
    data = with(res_trim_fill, tibble(gi = yi[fill], sei = sqrt(vi[fill]))),
    color = trim_fill_color, shape = 0
  )

# Save the plot
ggsave(here(figures_dir, "funnel_trim_fill.pdf"), width = 6, height = 3.5)
```
...
summ_brm <- list(intercept = res_trim_fill$beta)
# Compute funnel
...
# # Create funnel plot
plotdata <- as.data.frame(list(yi = res_trim_fill$yi, vi = sqrt(res_trim_fill$vi)))
...
sel_mod <- selmodel(res_freq_2, type = "stepfun", steps = c(0.01, .05, .3, 1), verbose = TRUE)
print(sel_mod)
...
sel_moderateonetailed <- selmodel(res_freq_2, type = "stepfun", steps = tab$steps, delta = tab$delta.mod.1, verbose = TRUE)
sel_severeonetailed <- selmodel(res_freq_2, type = "stepfun", steps = tab$steps, delta = tab$delta.sev.1, verbose = TRUE)
sel_moderatetwotailed <- selmodel(res_freq_2, type = "stepfun", steps = tab$steps, delta = tab$delta.mod.2, verbose = TRUE)
sel_severetwotailed <- selmodel(res_freq_2, type = "stepfun", steps = tab$steps, delta = tab$delta.sev.2, verbose = TRUE)
