...
# Add footnotes
colnames(tabs8)[7] <- str_c(colnames(tabs8)[7], "\\textsuperscript{a}")
tabs8$` `[8] <- str_c(tabs8$` `[8], "\\textsuperscript{b}")
tabs8$` `[9] <- str_c(tabs8$` `[9], "\\textsuperscript{c}")
tabs8$` `[10] <- str_c(tabs8$` `[10], "\\textsuperscript{d}")
tabs8$` `[11] <- str_c(tabs8$` `[11], "\\textsuperscript{e}")
...
apa_table(
  ...
  note = str_c(
    "\\scriptsize",
    "\\textsuperscript{a} = violation of expectation, ",
    "\\textsuperscript{b} = an uninformative (uniform) prior on the mean effect, ",
    "\\textsuperscript{c} = an informative (normal) prior on the mean effect, ",
    "\\textsuperscript{d} = an uninformative (uniform) prior on the standard deviations, ",
    "\\textsuperscript{e} = an informative (Student-$t$) prior on the standard deviations."
...
```{r, figs1, include=TRUE, fig.height=8, fig.width=12, fig.cap="(ref:figs1-caption)"}
...
(ref:figs2-caption) **Literature search and selection process.** (A) We searched four online databases as well as reviews and reference sections to identify articles in which mental rotation experiments in infants were reported. (B) Experiments were included in the meta-analysis if they fulfilled six pre-specified inclusion criteria. (C) Redundant articles comprising the same experiment(s) were excluded. \textsuperscript{a} = We decided to exclude two additional articles: one article because it was based on an uncommon experimental paradigm, differing substantially from all other articles, and another article because it was based on a sample of infants who were substantially older compared to all other articles.
...
spread_draws(res_meta_pos, `b_.*`, `sd_.*`, regex = TRUE, ndraws = NULL) %>%
...
(summ_meta_pos <- as.list(mean_qi(draws_meta_pos)))
```

```{r, figs5new, include=TRUE, fig.height=14, fig.width=12, fig.cap="(ref:figs5new-caption)"}
# Get posterior draws for the effect *in each experiment*
...
# Save the plot
ggsave(here(figures_dir, "figs5new_meta_pos.pdf"), width = 12, height = 14)
```

(ref:figs5new-caption) **Meta-analysis of mental rotation performance, accounting for familiarity preferences.** Analysis and formatting are the same as in Figure 2, but with effect sizes that indicated a familiarity preference (i.e., longer looking times at the familiar stimulus as compared to the novel stimulus) being re-coded as providing positive evidence for (preliminary) mental rotation ability. 
...
# Extract posterior draws for the meta-analytic effects
spread_draws(res_gender_pos, `b_.*`, `sd_.*`, regex = TRUE, ndraws = NULL) %>%
  # Compute variances and ICC from standard deviations
  mutate(
    intercept = b_Intercept,
    sigma_article = sd_article__Intercept,
...
# Summarize as means and 95% credible intervals
(summ_gender_pos <- as.list(mean_qi(draws_gender_pos)))
```

```{r, figs6new, include=TRUE, fig.height=8, fig.width=12, fig.cap="(ref:figs6new-caption)"}
# Get posterior draws for the effect *in each experiment*
...
# Save the plot
ggsave(here(figures_dir, "figs6new_gender_pos.pdf"), width = 12, height = 8)
```

(ref:figs6new-caption) **Meta-analysis of gender differences, accounting for familiarity preferences.** Analysis and formatting are the same as in Supplementary Figure 4, but with one effect size [@moore2011] re-coded as a male advantage (rather than as a female advantage) in mental rotation performance. Male infants in this study showed a familiarity preference (i.e., longer looking times at the familiar stimulus as compared to the novel stimulus), which may be interpreted as preliminary mental rotation ability, while female infants showed no discrimination between stimuli.
