...
+----------------------------------------------------------------+---------------------------------------------------------------------------------------------+
| \ \ (5\) Mean looking time and ${SD}$ per condition            | ${SD}_\text{av}=\sqrt{\frac{{SD}_\text{novel}^2+{SD}_\text{familiar}^2}{2}}$;\quad          |
| [@cumming2012]                                                 | $d=\frac{m_\text{novel}-m_\text{familiar}}{{SD}_\text{av}}$;\quad then (1)                  |
+----------------------------------------------------------------+---------------------------------------------------------------------------------------------+
...
if (run$sensitivity_analysis) {
  ...
  write_csv(tabs7, file = here(tables_dir, "tabs7_sens_meta.csv"))
  ...
  write_csv(tabs8, file = here(tables_dir, "tabs8_sens_reg.csv"))
  ...
  write_csv(tabs9, file = here(tables_dir, "tabs9_sens_gender.csv"))
}
```
...
tabs7 <- read_csv(
  here(tables_dir, "tabs7_sens_meta.csv"),
...
# Load sensitivity analysis table from file
tabs8 <- read_csv(
  here(tables_dir, "tabs8_sens_reg.csv"),
  col_types = "c", na = character(), trim_ws = FALSE
)
...
# Load sensitivity analysis table from file
tabs9 <- read_csv(
  here(tables_dir, "tabs9_sens_gender.csv"),
  col_types = "c", na = character(), trim_ws = FALSE
)
...
map(names(res_freq_2_lvl), function(name) {
  ...
  tibble(
    ...
    label_2 = c(
      "--", "--", "Weight function [0.01, 0.05, 0.30]",
...
write_csv(tabs10, file = here(tables_dir, "tabs10_correction_meta.csv"))
write_csv(tabs11, file = here(tables_dir, "tabs11_correction_gender.csv"))
```
