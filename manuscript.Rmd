...
header-includes:
  ...
  - \captionsetup[table]{font=small,labelfont=bf,labelsep=bar,textfont=bf,textformat=tabletext}
...
dir.create(here("results", "models"), showWarnings = TRUE)
n_iter <- 4000
n_warmup <- 1000
n_chains <- 4
res_prior <- brm(
...
list(dat, dat_gender) %>%
  map(function(plot_dat) {
    ...
    # Compute two different funnels under the null hypothesis
    z_crit_05 <- stats::qnorm(0.975)
    funnel_05 <- data.frame(
    ...
    funnel_01 <- data.frame(
      x = c(0 - z_crit_01 * sqrt(max_se^2), 0, 0 + z_crit_01 * sqrt(max_se^2)),
      y = c(max_se, 0, max_se)
    )

    # Compute funnel around the observed meta-analytic effect size
    funnel_observed <- mutate(funnel_05, x = x + summ_brm$intercept)

    # Plot the funnel(s)
    ggplot(plot_dat, aes(x = gi, y = sei)) +
      # Null funnels
      geom_polygon(
        data = funnel_01, aes(x = x, y = y), fill = "grey60", alpha = 0.2
      ) +
      geom_polygon(
        data = funnel_05, aes(x = x, y = y), fill = "grey60", alpha = 0.2
      ) +
      # Grid lines
...
+=======================================================================+==========================================================================================+
| (1) Cohen's $d$ and sample size $n$                                   | $df=2(n-1)$;\quad                                                                        |
| [@goulet-pelletier2018; @hedges1985]                                  | $J(df)=\frac{\Gamma(\frac{1}{2}df)}{\sqrt{\frac{df}{2}}\Gamma(\frac{1}{2}(df-1))}$;\quad |
|                                                                       | $g=d*J(df)$                                                                              |
+-----------------------------------------------------------------------+------------------------------------------------------------------------------------------+
| (2) $t$-statistic and sample size $n$                                 | $d=\frac{t}{\sqrt{n}}$;\quad                                                             |
| [@rosenthal1991]                                                      | then (1)                                                                                 |
+-----------------------------------------------------------------------+------------------------------------------------------------------------------------------+
| (3) Mean difference $m_\text{diff}$ and $SD_\text{diff}$              | $d=\frac{m_\text{diff}}{SD_\text{diff}}$;\quad                                           |
| [@lakens2013]                                                         | then (1)                                                                                 |
+-----------------------------------------------------------------------+------------------------------------------------------------------------------------------+
| (4) Mean looking times per condition + $SD$s                          | $SD_\text{av}=\sqrt{\frac{SD_\text{novel}^2+SD_\text{familiar}^2}{2}}$;\quad             |
| [@cumming2012]                                                        | $d=\frac{m_\text{movel}-m_\text{familiar}}{SD_\text{av}}$;\quad                          |
|                                                                       | then (1)                                                                                 |
+-----------------------------------------------------------------------+------------------------------------------------------------------------------------------+
| (5) Mean novelty preference score $m_\text{pref}$ + $SD_\text{pref}$  | $d=\frac{m_\text{pref}}{SD_\text{pref}}$;\quad                                           |
| [@lakens2013]                                                         | then (1)                                                                                 |
+-----------------------------------------------------------------------+------------------------------------------------------------------------------------------+
...
```{r, tab1, eval=FALSE, include=TRUE}
# Print table for conversion of effect sizes
tibble(
  `Available statistics` = c(
    "(1) Cohen's $d$ and sample size $n$ [@goulet-pelletier2018]",
    "(2) $t$-statistic and sample size $n$",
    "(3) Mean difference $m_\\text{diff}$ and $SD_{m_\\text{diff}}$",
    "(4) Mean looking times per condition and their $SD$s",
    "(5) Mean novelty preference score $m_\\text{pref}$ and its $SD$"
  ),
  `Conversion to standardized mean difference (Hedges' $g$)` = c(
    paste(
      "$df=2(n-1)$",
      "$j=\\frac{\\Gamma(\\frac{1}{2}v)}{\\sqrt{\\frac{v}{2}}\\Gamma(\\frac{1}{2}(v-1))}$",
      "$g=d*j$",
      sep = "\\newline"
    ),
    "$d=\\frac{t}{\\sqrt{n}}$; then (1)",
    "$d=\\frac{m_\\text{diff}}{SD_{m_\\text{diff}}}$, then (1)",
    paste(
      "$SD_\\text{av}=$",
      "$d=\\frac{m_\\text{novel}-m_\\text{familiar}}{}$",
      sep = "\\newline"
    ),
    ""
  )
  ...
  apa_table(
    caption = "Extracted between-article summary statistics and conversion to effect sizes",
...
One outcome measure per experiment was converted into a standardized mean difference with small sample correction (Hedges’ g) using the formulas provided in Table 1. The standard error $SE_g$ of the outcome measure $g$ for each experiment was computed using the formula provided by @goulet-pelletier2018:

$$SE_g=\sqrt{\frac{df}{df-2}\frac{2(1-r)}{n}(1+g^2\frac{n}{2(1-r)})-\frac{g^2}{J^2}}$$

where $n$ is the sample size of the experiment, $df$ are the degrees of freedom (with $df = 2*(n-1)$), $r$ is the correlation between the two dependent measures in the experiment, and $J(df)$ is the correction factor for small samples as described in Table 1. The correlation $r$ was not reported in any of the original articles [@harrer2021]. We therefore always assumed a correlation of $r = .50$ to make our analysis comparable to standard (between-group) meta-analyses [@cohen1988; @lakens2013] and because we were able to infer an average correlation of $r \approx .50$ from a subsample of articles which provided sufficient information (Supplementary Methods 1). A sensitivity analysis indicated that changing the assumed correlation to values from $r = -.90$ via $r = .00$ to $r = .90$ had no meaningful impact on the meta-analytic effect size (Supplementary Table 5).
...
| Available statistics                                                  | Conversion to standardized mean difference (Hedges' $g$)                             |
+=======================================================================+======================================================================================+
| (1) Cohen's $d$ and group sample size $n$                             | $df=n_{female}+ n_{male}-1$; \quad                                                   |
| [@goulet-pelletier2018; @hedges1985]                                  | $j=\frac{\Gamma(\frac{1}{2}df)}{\sqrt{\frac{df}{2}}\Gamma(\frac{1}{2}(df-1))}$;\quad |
|                                                                       | $g=d*j$                                                                              |
+-----------------------------------------------------------------------+--------------------------------------------------------------------------------------+
| (2) $t$-statistic and group sample size $n$                           | $d = t \sqrt{\frac{1}{n_{female}} + \frac{1}{n_{male}$;\quad                         |
| [@lankens2013]                                                        | then (1)                                                                             |
+-----------------------------------------------------------------------+--------------------------------------------------------------------------------------+
| (3) $F$ statistic and group sample size                               | $d = \sqrt{f} \sqrt{\frac{1}{n_{female}} + \frac{1}{n_{male}$;\quad                  |
| [@lakens2013]                                                         | then (1)                                                                             |
+-----------------------------------------------------------------------+--------------------------------------------------------------------------------------+
| (4) Mean difference in looking times per group + $SD$s                | $SD_\text{mean} = \sqrt{\frac{{(n_{female} - 1) \sigma^2_{female} + (n_{male} -1) \sigma^2_{male}}}{df}}$;\quad |
| [@lankens2013]                                                        | $d=\frac{m_\text{female}-m_\text{male}}{SD_\text{mean}}$;\quad                       |
|                                                                       | then (1)                                                                             |
+-----------------------------------------------------------------------+--------------------------------------------------------------------------------------+
| (5) Mean novelty preference score $m_\text{pref}$ + $SD_\text{pref}$  | $SD_\text{pref} = \sqrt{\frac{{(n_{female} - 1) \sigma^2_{female} + (n_{male} -1) \sigma^2_{male}}}{df}}$;\quad |
|  [@lakens2013]                                                        | $d=\frac{m_\text{female}-m_\text{male}}{SD_\text{pref}}$;\quad                       |
|                                                                       | then (1)                                                                             |
+-----------------------------------------------------------------------+--------------------------------------------------------------------------------------+
...
$    \sigma_{between group} = \sqrt{\frac{df}{df-2} \frac{2}{\tilde{n}} (1 + g^2\frac{\tilde{n}}{2}) - frac{g^2}{J^2}}          $
Where ñ is the harmonic mean of the group sizes and $ñ = \frac{n_{female} n_{male}}{n_{female} + n_{male}}$ and $df$ are given by the equation in Table xxx.
...
Publication bias was evaluated based on a funnel plot and an Egger regression test [@egger1997; @sterne2005]. To produce the funnel plot, which visualizes the relationship between standard errors and effect sizes, we adapted code from the R package metaviz (`r packageVersion("metaviz")`) [@kossmeier2020]. For the Egger regression test we used the R package metafor (Version `r packageVersion("metafor")`) [@viechtbauer2010] to identify a potential relationship between standard errors and effect sizes. Such a relationship is indicated by the observation that the slope of a weighted linear regression of the effect sizes on the standard errors is significantly different from zero in a two-sided $t$-test.

To scrutinize the robustness of the meta-analytic effect size against the effects of individual studies we conducted a jackknife leave-one-out-analysis [@gee2005; @efron1993]. To this end, we adapted code from the bootstrap R package (Version `r packageVersion("bootstrap")`) [@kostyshak2019]. Specifically, we fit the Bayesian models repeatedly to our study samples, leaving out one experiment sample for every iteration. To take into account the variance inherent in Bayesian model estimation, we averaged over ??? iterations of the jackknife. 

## Data availability

The data is available via (OSF link here).

## Code availability

The code is available via (OSF link here).
