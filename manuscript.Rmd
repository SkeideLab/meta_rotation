...
The probability for this effect being larger than zero was `r print_prob(hyps_brm_reg$male_female$Post.Prob)`.
We found no difference between mixed-gender and all-female samples, $b = `r -summ_reg$female_mixed`$, 95% CrI [`r -summ_reg$female_mixed.lower`, `r -summ_reg$female_mixed.upper`] (`r print_prob(hyps_brm_reg$female_mixed$Post.Prob)` probability of an effect larger than zero).
...
The probability for this effect being greater than zero was `r print_prob(hyps_brm_reg$voe_habituation$Post.Prob)`.
...
To confirm the gender difference between males and females, we set up another Bayesian meta-analysis, this time focusing on the looking-time contrasts between male and female infants reported *within* each experiment by the original authors.
Our additional analysis revealed a meta-analytic effect size of $g = `r summ_gender$intercept`$, 95% Crl [`r summ_gender$intercept.lower`, `r summ_gender$intercept.upper`] (Supplementary Fig.\ \@ref(fig:figs1)) and a probability of this effect being greater than zero of `r print_prob(hyp_gender$Post.Prob)`.
...
One recent study on 5- to 6-month-old female infants provided preliminary evidence for possible social-environmental effects related to parental attitudes towards gender which might partly explain the results of our present work [@constantinescu2018]. As far as we know, potentially mediating and moderating factors that could already be operational in infancy, however, are not yet empirically established. In a similar vein, while mental rotation training has small-to-medium post-test effects in children, it is unclear whether it can remove gender differences and be adapted to infants [@uttal2013].
...
In the present meta-analysis, we followed the established PRISMA 2020 guidelines (Preferred Reporting Items for Systematic reviews and Meta-Analyses) [@page2021]. The PRISMA flowchart is provided in Supplementary Fig.\ \@ref(fig:figs2) while Supplementary Table\ \@ref(tab:tabs4) contains the PRISMA checklist.
...
\noindent Supplementary Information is available for this paper. Correspondence and requests for materials should be addressed to Alexander Enge ([enge@cbs.mpg.de](mailto:enge@cbs.mpg.de)).
