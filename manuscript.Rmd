...
affiliation:
  ...
  - id            : "*"
    institution   : "Corresponding author (enge@cbs.mpg.de)"
...
```{r, fig1, include=TRUE, fig.height=14, fig.pos="bh", fig.width=12, fig.cap="(ref:fig1-caption)"}
...
As a next step, we conducted a meta-regression analysis to test if the gender of the infants, their age, or the type of mental rotation task was related to mental rotation performance (Fig.\ \@ref(fig:fig2)). Indeed, experiments with all-male samples revealed larger looking time differences than experiments with all-female samples, $b = `r summ_reg$male_female`$ (where $b = \Delta{g}$), 95% CrI [`r summ_reg$male_female.lower`, `r summ_reg$male_female.upper`]. The probability for this effect being larger than zero was `r print_prob(hyps_brm_reg$male_female$Post.Prob)`. We found no difference between mixed-gender and all-female samples, $b = `r -summ_reg$female_mixed`$, 95% CrI [`r -summ_reg$female_mixed.lower`, `r -summ_reg$female_mixed.upper`] (`r print_prob(hyps_brm_reg$female_mixed$Post.Prob)` probability of an effect larger than zero). Additionally, mean age was not related to mental rotation performance, with a change per year of $b = `r summ_reg$age`$, 95% [`r summ_reg$age.lower`, `r summ_reg$age.upper`]. We also did not detect an interaction between gender and age ([females - mixed] $\times$ age: $b = `r summ_reg$female_mixed_age`$, 95% CrI [`r summ_reg$female_mixed_age.lower`, `r summ_reg$female_mixed_age.upper`]; [males - females] $\times$ age: $b = `r summ_reg$male_female_age`$, 95% CrI [`r summ_reg$male_female_age.lower`, `r summ_reg$male_female_age.upper`]). Finally, there was weak evidence that violation of expectation tasks yielded larger effects than habituation tasks, $b = `r summ_reg$voe_habituation`$, 95% [`r summ_reg$voe_habituation.lower`, `r summ_reg$voe_habituation.upper`]. The probability for this effect being greater than zero was `r print_prob(hyps_brm_reg$voe_habituation$Post.Prob)`.
...
In the present meta-analysis, we followed the established PRISMA 2020 guidelines (Preferred Reporting Items for Systematic reviews and Meta-Analyses) [@page2021]. The PRISMA checklist is provided in Supplementary Table\ \@ref(tab:tabs4) while Supplementary Fig.\ \@ref(fig:figs2) displays the PRISMA flowchart.
...
We also performed trim-and-fill analyses and tested selection models to confirm the robustness of the results obtained from the frequentist meta-analyses against publication bias (Supplementary Methods 2; Supplementary Tables\ \@ref(tab:tabs10) and \@ref(tab:tabs11); Supplementary Fig.\ \@ref(fig:figs5)) [@duval2000; @vevea1995; @vevea2005].

## Data availability

The datasets generated and analyzed during the current study are available in an Open Science Framework repository [@enge2022a], [https://osf.io/k3wdg/?view_only=06a4a0ac3d5f4681baab11751c1498b8](https://osf.io/k3wdg/?view_only=06a4a0ac3d5f4681baab11751c1498b8). This link is intended for peer review only and will be replaced with a public DOI link upon publication.

## Code availability

The analysis code for the current study is available in an Open Science Framework repository [@enge2022a], [https://osf.io/k3wdg/?view_only=06a4a0ac3d5f4681baab11751c1498b8](https://osf.io/k3wdg/?view_only=06a4a0ac3d5f4681baab11751c1498b8). This link is intended for peer review only and will be replaced with a public DOI link upon publication.
...
Supplementary Information is available for this paper. Correspondence and requests for materials should be addressed to Alexander Enge ([enge@cbs.mpg.de](mailto:enge@cbs.mpg.de)).
