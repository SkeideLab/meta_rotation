...
header-includes:
  ...
  - \captionsetup[table]{belowskip=20pt,font=small,labelfont=bf,labelsep=colon,textfont=bf,textformat=tabletext}
  - \captionsetup[figure]{belowskip=10pt,font=small,labelfont=bf,labelsep=colon,name=Fig.,textfont=normalfont}
---
...
We inspected funnel plots and performed Egger regression tests to examine the possibility of publication bias in the literature that we had meta-analyzed. For the meta-analysis of mental rotation performance, we observed a slight asymmetry in the funnel plot, indicating a small publication bias (Fig. 3). This was confirmed by an Egger regression test that showed a reliable association between experiments' effect sizes and their corresponding standard errors, $b = `r coef(egger$fit)["Xsei"]`$, $t(`r egger$dfs`) = `r egger$zval`$, $p = `r print_p(egger$pval)`$, 95% confidence interval (CI) [`r egger_ci["2.5 %"]`, `r egger_ci["97.5 %"]`] (two-sided test). Nevertheless, a jackknife (leave-one-out analysis) confirmed that the current results are robust to the effects of individual outlier studies (Supplementary Table 3). For the meta-analysis of gender differences in mental rotation performance, the asymmetry in the funnel plot was less pronounced and the slope of the Egger regression test was not statistically significant, $b = `r coef(egger_gender$fit)["Xsei"]`$, $t(`r egger_gender$dfs`) = `r egger_gender$zval`$, $p = `r print_p(egger_gender$pval)`$, 95% confidence interval (CI) [`r egger_ci_gender["2.5 %"]`, `r egger_ci_gender["97.5 %"]`] (two-sided test).
...
To scrutinize the robustness of the meta-analytic effect size against the influence of any individual experiment (which may or may not be a false-positive), we conducted a jackknife leave-one-out-analysis for the meta-analysis of mental rotation performance [@gee2005; @efron1993]. To this end, we adapted code from the bootstrap R package (Version `r packageVersion("bootstrap")`) [@kostyshak2019]. Specifically, we refitted the Bayesian three-level model repeatedly while leaving out a different one of the original experiments on every iteration. To take into account the variance inherent in Bayesian model estimation, we averaged over $x$ iterations of the jackknife.

We also ran trim-and-fill analyses and selection models to confirm the robustness of the results from the frequentist meta-analyses against publication bias (Supplementary Methods 2; Supplementary Tables 8 and 9) [@duval2000; @vevea1995; @vevea2005].

## Data availability

The data for this study is available via the [Open Science Framework](https://osf.io/k3wdg/?view_only=06a4a0ac3d5f4681baab11751c1498b8) [@enge2022a]. Note that this link is intended for peer review only and will be replaced with a public link upon acceptance.
