...
dat %>%
  select(id, group, yi_type, d, d_z_t, d_z_diff, d_av, d_nov_pref, ri) %>%
  print(n = Inf)
...
dat %>%
  ...
  select(id, group, ni, yi, sei) -> dat_r

# Create vector of experiment IDs for plotting
experiment_ids <- with(dat_r, paste(id, group, sep = ", "))

# Add sampling variance based standard error
escalc(vi = vi, sei = sei, ni = ni, data = dat_r)
...
# res_age <- rma.mv(
#   yi, vi,
#   mods = ~age_mean, random = ~ group | id, data = dat, slab = slab
# )
# print(res_age)
# forest(res_age)
...
# res_gender <- rma.mv(
#   yi, vi,
#   mods = ~female_percent, random = ~ group | id, data = dat, slab = slab
# )
# print(res_gender)
# forest(res_gender)
...
# res_full <- rma.mv(
#   yi, vi,
#   mods = ~age_mean * female_percent, random = ~ group | id, data = dat, slab = slab
# )
# print(res_full)
# forest(res_full)
```
