...
draws <- epred_draws(res_brm, dat, ndraws = n_draws) %>%
  mutate(experiment_f = factor(experiment))

# Get posterior draws for the meta-analytic effect
meta_draws <- spread_draws(res_brm, b_Intercept, ndraws = n_draws)
...
dat %>%
  ...
  geom_vline(xintercept = seq(-3, 3, 1), size = 0.8, color = "grey90") +
  geom_vline(xintercept = seq(-2.5, 2.5, 1), size = 0.4, color = "grey90") +
  ...
  geom_point(aes(size = ni), shape = 15) + # Or (1 / vi)?
...
format_days_years <- function(days) {
  days_per_year <- 365.25
  days_per_month <- 30.417
  years <- days %/% days_per_year
  months <- round((days - years * days_per_year) / days_per_month)
  str <- paste0(as.character(years), "y ", as.character(months), "m")
  str[is.na(days)] <- NA
  return(str)
}
format_days_months <- function(days, digits = 1) {
  days_per_month <- 30.417
  months <- days / days_per_month
  str <- format(round(months, digits), trim = TRUE, nsmall = digits)
  str <- paste0(str, "m")
  str[is.na(days)] <- NA
  return(str)
}
...
dat %>%
  transmute(
    ...
    `Mean age^a` = format_days_years(age_mean),
    SD = format_days_months(age_sd),
    Task = task,
...
dat %>%
  mutate(
    ...
    # Re-code as contrast for the meta-regression model
    gender = recode(gender_f, "Mixed" = 0, "Female" = -0.5, "Male" = 0.5),
    ...
    task_f = factor(task, levels = c("Habituation", "VoE")),
    # Re-code as contrast for the meta-regression model
    task = recode(task_f, "Habituation" = -0.5, "VoE" = 0.5),
    .keep = "unused"
  ) -> dat_reg
...
alph <- 0.7
dat_reg %>%
  ggplot(aes(x = age * 12, y = gi, color = gender_f)) +
  # Data points
