...
draws_reg_hdi <- mean_hdi(draws_reg)
...
dat_reg %>%
  ...
  # Regression lines - Habituation tasks
  geom_lineribbon(
    aes(y = .epred, ymin = .lower, ymax = .upper, color = gender_task),
    data = draws_reg_hdi,
    fill = NA
  ) +
  # Bayesian HPDIs - Habituation tasks
  geom_ribbon(
    aes(ymin = .lower, ymax = .upper, color = gender_task),
    data = draws_reg_hdi,
    fill = NA
  ) +
  ...
  # Regression lines - VoE tasks
  geom_lineribbon(
    aes(y = .epred, ymin = .lower, ymax = .upper, color = gender_task),
    data = draws_reg_hdi,
    fill = NA
  ) +
  # Bayesian HPDIs - VoE tasks
  geom_ribbon(
    aes(ymin = .lower, ymax = .upper, color = gender_task),
    data = draws_reg_hdi,
    fill = NA
  ...
  theme(
    ...
    panel.grid = element_blank(), # element_line(color = "grey90"),
    text = element_text(family = "Helvetica"),
  ) -> plot_reg
...
plot_grid(
  ...
  draw_plot(plot_reg_legend, x = 0.42, y = 0.84, hjust = 0.5, vjust = 0.5)
