...
seq(-0.9, 0.9, by = 0.3) %>%
  map_dfr(function(ri_sens) {
    ...
    tidy_draws(res_ri_sens) %>%
      mutate(ri_sens = ri_sens)
  }) -> draws_ri_sens

# Plot meta-analytic results for each assumed correlation
draws_ri_sens %>%
  mutate(ri_sens_f = fct_rev(factor(print_num(ri_sens)))) %>%
  ggplot(aes(y = ri_sens_f, x = b_Intercept)) +
  geom_vline(xintercept = seq(-0.75, 0.75, 0.25), color = "grey90") +
  stat_halfeye(point_interval = "mean_qi", .width = c(0.5, 0.95)) +
  coord_cartesian(xlim = c(-0.75, 0.75), clip = "off") +
  scale_x_continuous(breaks = seq(-0.75, 0.75, 0.25)) +
  labs(x = expression("Hedges" ~ italic("g"))) +
  annotate(
    "text",
    label = "Correlation", x = -1.1, y = 7.86, hjust = 0,
    family = "Helvetica", fontface = "bold", color = "black"
  ) +
  theme_minimal() +
  theme(
...
# Save the plot
ggsave(here(figures_dir, "ri_sens.pdf"), width = 4, height = 5)
```
