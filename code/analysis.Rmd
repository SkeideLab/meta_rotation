...
included %>%
  mutate(
    ...
    age = age_months - mean(age_months, na.rm = TRUE),
...
(contrasts(dat_both_mixed$gender) <- MASS::contr.sdif(3))
...
level <- 0.05
ndraws <- 1000
dat_plot <- dat_both_mixed
res_plot <- res_ml
res_plot_brm <- res_brm
...
draws <- epred_draws(res_brm, dat_plot, ndraws = ndraws)
...
dat_plot %>%
  ...
  bind_rows(list(
    ...
    gi = res_plot$b,
    ci_lb = res_plot$ci.lb,
    ci_ub = res_plot$ci.ub,
    fontface = "bold"
  )) %>%
  ...
  annotate("segment", x = res_plot$b, xend = res_plot$b, y = 0.7, yend = 1.3) +
  annotate("errorbar", xmin = res_plot$ci.lb, xmax = res_plot$ci.ub, y = 1.0) +
  # Styling
...
theta <- function(x, dat_r) {
  rma.mv(
    yi, vi,
    mods = ~age_months,
    random = ~ group | id,
    data = dat_r[x, ],
    slab = experiment_ids[x]
  )
}

res_jk <- jackknife(1:nrow(dat_r), theta, dat_r)
```
# Methods
