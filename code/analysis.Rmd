...
Indeed, experiments with all-male samples revealed larger looking time differences than experiments with all-female samples, b = 0.48, 95% CrI [0.06, 0.91].
The probability for this effect being larger than zero was 98.5%.
We found no difference between mixed-gender and all-female samples, b = 0.13, 95% CrI [-0.20, 0.52] (75.4% probability of an effect larger than zero).
Additionally, mean age was not related to mental rotation performance, with a change per year of b = 0.05, 95% [-0.58, 0.68].
We also did not detect an interaction between gender and age ([females - mixed] × age: b = 0.02, 95% CrI [-0.81, 0.84]; [males - females] × age: b = 0.06, 95% CrI [-0.84, 0.96]).
...
To confirm the gender difference between males and females we set up another Bayesian meta-analysis, this time focusing on the looking-time contrasts between all-male and all-female groups reported within each article by the original authors.
Our additional analysis revealed a meta-analytic effect size of g = 0.20, 95% Crl [0.03, 0.38] (Supplementary Fig. 1) and a probability of this effect being greater than zero of 98.9%.
The heterogeneity of effect sizes was \$sigma\$2Experiment = 0.01, 95% CrI [0.00, 0.07], at the experiment level, and \$sigma\$2Article = 0.02, 95% CrI [0.00, 0.10] at the article level.
This means that 56.1% of the total variance can be attributed to differences between articles and 43.1% to differences within articles.

## Evaluation of publication bias

By visual inspection of a funnel plot of standard errors and effect sizes we detected a slight asymmetry indicating a small publication bias (Fig. 3).
Publication bias was also suggested by the results of an Egger regression test indicating that the slope of a weighted linear regression of the effect sizes on the standard errors is significantly different from zero (t(57) = 3.37, p = 0.001).
Nevertheless, a jackknife leave-one-out analysis confirmed that the current results are robust to the effects of individual studies (see Supplementary Table 5).
