...
table(dat$gender_split)

# Extract mixed/split experiments only
dat_mixed <- filter(dat, gender_split %in% c("mixed", "mixed_only"))
dat_split <- filter(dat, gender_split %in% c("split", "split_only"))

# Extract both strategies combined (but excluding any redundant experiments)
dat_both <- filter(
  dat, gender_split %in% c("mixed", "mixed_only", "split_only")
)
...
dat_both %>%
  ...
  select(article, group, age_months, female_percent, ni, yi, sei) %>%
...
experiment_ids <- with(dat_r, paste(article, group, sep = ", "))
...
# profile(res_mv, tau2 = 1)
# profile(res_mv, rho = 1)

# Meta-regression with age
res_age <- rma.mv(
  yi, vi,
  mods = ~age_months,
  random = ~ group | article,
  data = dat_r,
  slab = experiment_ids
)
print(res_age)
