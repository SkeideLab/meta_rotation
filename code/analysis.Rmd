...
dat %>%
  ...
  geom_text(aes(x = -5.6, label = article), hjust = 0) +
  geom_text(aes(x = -3.1, label = group), hjust = 0) +
  # Add experiment-specific effect sizes and CIs as text on the right
  geom_text(aes(x = 4.4, label = ci_print), hjust = 1) +
  ...
  annotate(
    ...
    y = -0.5,
    label = print_ci(
      meta_effect_size$Estimate,
      meta_effect_size$`l-95% CI`,
      meta_effect_size$`u-95% CI`
    ),
    hjust = 1,
    fontface = "bold"
  ) +
  ...
  annotate(
    ...
    y = n_experiments + 1.5,
    label = c(
      "Article", "Experiment", "Effect size plot", "Effect size statistic"
    ),
    hjust = c(0, 0, 0.5, 1),
    fontface = "bold"
  ) +
  ...
  theme(
    ...
    axis.title.x = element_text(hjust = 0.565),
